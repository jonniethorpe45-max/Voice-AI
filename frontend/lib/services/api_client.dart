...
class ApiClient {
  ...
  Future<void> process({
    ...
    required StyleControls controls,
    required List<String> preferredVariations,
  }) async {
    final response = await http.post(
      ...
      body: jsonEncode({
        'job_id': jobId,
        'style_controls': controls.toJson(),
        'preferred_variations': preferredVariations,
      }),
    );
    if (response.statusCode >= 400) {
  ...
  Future<JobResults> results(String jobId) async {
    final response = await http.get(_uri('/results/$jobId'));
    if (response.statusCode >= 400) {
      throw Exception('Results failed (${response.statusCode}): ${response.body}');
    }
    return JobResults.fromJson(jsonDecode(response.body) as Map<String, dynamic>);
  }
